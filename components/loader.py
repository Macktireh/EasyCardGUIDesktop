...
class Loader(CTkFrame):
    def __init__(
        ...
        master: CTkBaseClass,
        text: str = "Loading...",
        fg_color: str = Color.BG_CONTENT,
        bg_color: str = Color.BG_CONTENT,
    ) -> None:
        self.master = master

        super().__init__(self.master, fg_color=fg_color, bg_color=bg_color)

        self.loader = AnimatedGif(
            self,
            gif_file=AssetsImages.LOADING_GIF,
            delay=0.02,
            bg=Color.BG_CONTENT[1] if self._get_appearance_mode() == "dark" else Color.BG_CONTENT[0],
        )
