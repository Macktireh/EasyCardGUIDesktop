...
class AnimatedGif(tk.Label):
    ...
    def __init__(self, root, gif_file, delay=0.04, **kwargs):
        """
        ...
        self._stop = False  # Thread exit request flag
    ...
    def _animate(self):
        try:
            self.gif = tk.PhotoImage(file=self.gif_file, format=f"gif -index {self._num}")  # Looping through the frames
            ...
            self._num = 0
        if not self._stop:  # If the stop flag is set, we don't repeat
    ...
    def _animate_thread(self):
        ...
        while (
            self._stop is False
        ):  # Normally this would block mainloop(), but not here, as this runs in separate thread  # noqa: E501
            try:
                time.sleep(self.delay)
                self.gif = tk.PhotoImage(
                    file=self.gif_file, format=f"gif -index {self._num}"
                )  # Looping through the frames  # noqa: E501
                self.configure(image=self.gif)
                self._num += 1
            except tk.TclError:  # When we try a frame that doesn't exist, we know we have to start over from zero
                self._num = 0
